...
C assign growth functions Lambda
        ...
        EE = 1+2*Theta2+Theta2**2+Theta2**4.0/25+2.0/5
     &  *Theta2**2*(1+Theta2)*Cos(9*Theta1)
        FF = 1.0/25*(1-4*Theta2)*Theta2*Sin(9*Theta1)
        GG = 17-8*Theta2+(626*Theta2**2)/625+2.0/25
     &  *Theta2*Cos(9*Theta1)
        LL = (25*(-4+Theta2)*(5+Theta2*(10+5*Theta2+2*Theta2**3)
     &  +11*Theta2**2*(1+Theta2)*Cos(9*Theta1)))
     &  /(Sqrt((125.0+125*Theta2+Theta2**3+5*Theta2*(1+6*Theta2)
     &  *Cos(9*Theta1))**2+625*(-4+Theta2)**2*(5*(1+Theta2)
     &  *Cos(Theta1)+Theta2**2*Cos(10*Theta1))**2+625
     &  *(-4+Theta2)**2*(5*(1+Theta2)*Sin(Theta1)+Theta2**2
     &  *Sin(10*Theta1))**2))
        MM = (25*(-4+Theta2)*Theta2*(2+Theta2)*Sin(9*Theta1))
     &  /(Sqrt((125.0+125*Theta2+Theta2**3+5*Theta2*(1+6*Theta2)
     &  *Cos(9*Theta1))**2+625*(-4+Theta2)**2*(5*(1+Theta2)
     &  *Cos(Theta1)+Theta2**2*Cos(10*Theta1))**2+625*(-4+Theta2)**2
     &  *(5*(1+Theta2)*Sin(Theta1)+Theta2**2*Sin(10*Theta1))**2))
        NN = (Sqrt(2.0)*(125+Theta2*(125+4*Theta2)+5*(4+Theta2
     &  *(4+5*Theta2))*Cos(9*Theta1)))/(Sqrt(531250.0+Theta2
     &  *(812500+Theta2*(62525+2*Theta2*(-93350+Theta2
     &  *(26325-5000*Theta2+626*Theta2**2))))+5*Theta2
     &  *(4*(125+Theta2*(10875+Theta2*(5750+Theta2*(-4374
     &  +631*Theta2))))*Cos(9*Theta1)+5*Theta2*(1+6*Theta2)**2
     &  *Cos(18*Theta1))))
...
C tranform FinalG from curvilinear coordinates to Cartesian coordiantes
        FinalG11 = (LamrR*X*X+LamtT*Y*Y)/MMOD
        FinalG22 = (LamrR*Y*Y+LamtT*X*X)/MMOD
        FinalG12 = (LamrR-LamtT)*X*Y/MMOD
        FinalG21 = FinalG12

        FinalG31 = -(LamzT*Y)/SQMMOD
        FinalG32 = (LamzT*X)/SQMMOD

        FinalG13 = -(LamtZ*Y)/SQMMOD
        FinalG23 = (LamtZ*X)/SQMMOD
