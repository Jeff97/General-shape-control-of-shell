...
C assign growth functions Lambda
        ...
        EE = 1.0/4*Pi**2*(1-2*Y)**2*Sin(2*Pi*X)**2+(1.0/2+Sin(Pi*Y))**2
        FF = 1.0/4*Pi*((1-2*Y)*(2+Cos(2*Pi*X))*Sin(2*Pi*X)
     &          +(-1+2*X)*(Cos(Pi*Y)+Sin(2*Pi*Y)))
        GG = 1.0/4*((2+Cos(2*Pi*X))**2+Pi**2*(1-2*X)**2*Cos(Pi*Y)**2)
        LL = 0.0
        MM = 0.0
        NN = 0.0
...
!I cannot get total time from UMAT variable, please modify TotalT manually 
        ...
        DtltaG11 = ( Lambda1z0 + STATEV(3)*Lambda1z1
     &  -1.0)*(TIME(1)+DTIME)/TotalT
        DtltaG12 = ( LambdaSz0 + STATEV(3)*LambdaSz1
     &  )*(TIME(1)+DTIME)/TotalT
        DtltaG21 = DtltaG12
        DtltaG22 = ( Lambda2z0 + STATEV(3)*Lambda2z1
     &  -1.0)*(TIME(1)+DTIME)/TotalT
