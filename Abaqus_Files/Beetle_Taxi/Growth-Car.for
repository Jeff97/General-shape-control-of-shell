...
C
     ...
     &    FOUR=4.D0, SIX=6.D0, NELEMENT=64070)
...
C read {Er,Fr,Gr,Lr,Mr,Nr} of reference surface
        open(301,FILE='T:\Abaqus-Temp\20231201CarScale\'//
     &  'Lambda10.csv',status="old")
        read(301,*) Lambda1z0Imp
        close(301)

        open(302,FILE='T:\Abaqus-Temp\20231201CarScale\'//
     &  'Lambda20.csv',status="old")
        read(302,*) Lambda2z0Imp
        close(302)

        open(303,FILE='T:\Abaqus-Temp\20231201CarScale\'//
     &  'Lambdas0.csv',status="old")
        read(303,*) LambdaSz0Imp
        close(303)

        open(304,FILE='T:\Abaqus-Temp\20231201CarScale\'//
     &  'Lambda11.csv',status="old")
        read(304,*) Lambda1z1Imp
        close(304)

        open(305,FILE='T:\Abaqus-Temp\20231201CarScale\'//
     &  'Lambda21.csv',status="old")
        read(305,*) Lambda2z1Imp
        close(305)

        open(306,FILE='T:\Abaqus-Temp\20231201CarScale\'//
     &  'Lambdas1.csv',status="old")
        read(306,*) LambdaSz1Imp
        close(306)

C read components of the two tengent vectors
        open(501,FILE='T:\Abaqus-Temp\20231201CarScale\'//
     &  'a1.csv',status="old")
        read(501,*) a1Imp
        close(501)

        open(502,FILE='T:\Abaqus-Temp\20231201CarScale\'//
     &  'a2.csv',status="old")
        read(502,*) a2Imp
        close(502)

        open(503,FILE='T:\Abaqus-Temp\20231201CarScale\'//
     &  'a3.csv',status="old")
        read(503,*) a3Imp
        close(503)

        open(504,FILE='T:\Abaqus-Temp\20231201CarScale\'//
     &  'b1.csv',status="old")
        read(504,*) b1Imp
        close(504)

        open(505,FILE='T:\Abaqus-Temp\20231201CarScale\'//
     &  'b2.csv',status="old")
        read(505,*) b2Imp
        close(505)

        open(506,FILE='T:\Abaqus-Temp\20231201CarScale\'//
     &  'b3.csv',status="old")
        read(506,*) b3Imp
        close(506)
...
C TODO: growth functions in the curvilinear coordinates for test
      !   Lam11 = 1.0
      !   Lam12 = 0.0
      !   Lam21 = Lam12
      !   Lam22 = 1.0
